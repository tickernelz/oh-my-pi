...
export class AuthBrokerClient {
	...
	async deleteCredentialBlockIfUnchanged(
		...
		// A static facade import would eagerly construct every wire schema during client startup.
		const { credentialBlockDeleteResponseSchema } = await import("./wire-schemas");
	...
	async #request<t>(
		...
		path: string,
		opts: {
			schema: AuthBrokerResponseSchemaName | ((input: unknown) => unknown);
			auth?: boolean;
			body?: unknown;
			signal?: AbortSignal;
		},
		...
		const text = await response.text();
		const raw = this.#parseJson(text, response.status);
		const schema = typeof opts.schema === "string" ? getAuthBrokerWireSchemas()[opts.schema] : opts.schema;
