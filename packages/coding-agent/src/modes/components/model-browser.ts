...
export class ModelBrowser implements Component {
	...
	#detailLines(width: number): [string, string] {
		...
		if (this.isOverContext(selected)) {
			const warning = `  ${theme.status.disabled} context ${formatNumber(this.#currentContextTokens).toLowerCase()} exceeds ${formatNumber(model.contextWindow ?? 0).toLowerCase()} limit ${theme.sep.dot} compacts with current model, then switches`;
