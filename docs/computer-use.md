...
While the tool is active, the system prompt routes host-desktop requests through `computer`, even with a compact tool inventory. It requires the agent to inspect each fresh screenshot before acting again. This does not enable the tool, bypass approval, or prevent a user-requested alternative after an error.
