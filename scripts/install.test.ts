...
describe("downstream shell installer", () => {
	it("defaults to the downstream release binary even when bun is installed", async () => {
	...
	it("--source clones and builds the downstream workspace without a global package install", async () => {
		const harness = await createHarness();
		await writeExecutable(
...
case "$1" in
  clone) mkdir -p "$last/packages/coding-agent" ;;
esac
`,
		);
		await writeExecutable(
...
`,
		...
		expect(result.exitCode).toBe(0);
		const calls = await Bun.file(harness.logPath).text();
		expect(calls).toContain("git clone --depth 1 --branch feature/lcm https://github.com/tickernelz/oh-my-pi.git");
