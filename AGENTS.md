...
- Every LCM change landed on downstream `main` MUST also be carried to the upstream PR branch `feat/lossless-context-management` in the same session, before the work is considered delivered. That branch is `upstream/main` plus exactly one squashed LCM feature commit, so port the LCM-owned files and its `[Unreleased]` changelog entries, then force-push with `--force-with-lease`. Verify the remote SHA equals the local HEAD afterwards.
- The PR branch MUST stay free of downstream-only artifacts: no `tickernelz` references, no `LCM_REVISION` provenance, no `downstream-release`/`upstream-sync` workflows, and no downstream `AGENTS.md` policy. Its package versions match the upstream base version. Audit with a grep for those markers plus a `git diff --stat upstream/main` review before pushing.
